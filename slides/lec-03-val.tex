...
\begin{small}
  \begin{tabular}{ll}
    ...
    \avm{< \textnormal{bet}, \ [\type*{word}
      head & \type{verb} \\
      % [\type*{agr-cat} agr & [ per & 3 \\ num & sg ] ]
      val & [ < NP (NP) (S) > ]
      ]>} \\
    \avm{< \textnormal{dine}, \ [\type*{word}
      head & \type{verb} \\
      % [\type*{agr-cat} agr & [ per & 3 \\ num & sg ] ]
      val & [ comps & < > ]
      ]>} &
    \avm{< \textnormal{fond}, \ [\type*{word}
      head & \type{adjective} \\
      % [\type*{agr-cat} agr & [ per & 3 \\ num & sg ] ]
...
\avm{[\type*{phrase}  val & [ comps & <  > ] ]
              \ $\rightarrow$\ \HD \ 
             [\type*{word}  head & \val{verb $|$ adj $|$ noun} \\
             val & [ comps & < \1, \ldots, \tag{n} > ] ]\ \ \1, \ldots, \tag{n}}

\avm{[\type*{phrase}  val & [ comps & <  > ] ]
              \ $\rightarrow$\ \ \ \1, \ldots, \tag{n}\ \ \HD \ 
             [\type*{word} head & \val{preposition} \\
             val & [ comps & < \1, \ldots, \tag{n} > ] ]}
...
\myslide{A possible formalization of the restriction}
   ...
   \avm{< \textnormal{insist}, \ [\type*{word}
      head & \textit{verb}\\
      % [\type*{agr-cat} agr & [ per & 3 \\ num & sg ] ]
      val & [ spr & < NP > \\
               comps & < S[ head & [  mood & subjunctive ] ] > ]
      ]>} %\[ \asort{verb} mood & subjunctive \]
...
[{phrase}]]
[{agr-cat \avm{[ \textsc{per,num} ]}} [{non-3sing} [{non-1sing} [{2sing}]
[{pl}]]
[{1sing}]]
[{3sing \avm{[ gend ]}}]]
[{val-cat \avm{[ \textsc{spr, comps} ]}}]
[{pos} [{agr-pos \avm{[ \textsc{agr} ]}} [{noun \avm{[ \textsc{case} ]}}]
[{verb \avm{[ \textsc{aux} ]}}]
[{det \avm{[ \textsc{count} ]}}]]
[{prep}]
[{adj}]
...
[{\avm{[\type*{phrase}\\
       HEAD & \0\\ 
       VAL & [ SPR & < >\\
                COMPS & < > ]]}} [{\@1\avm{[\type*{phrase}\\
		     HEAD & \4\\
		     VAL & [ SPR & < >\\
		              COMPS & < > ]]}} [{\@2\avm{[\type*{word}\\
					HEAD & [\type*{det}\\
						  AGR & \3\\
						  COUNT & + ]\\
					VAL & [ SPR & < >\\
