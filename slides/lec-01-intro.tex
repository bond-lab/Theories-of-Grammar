...
\item For all other issues of academic integrity, use your common sense and conscience.
  \begin{itemize}
  \item If you are not sure if something is ok, it is probably better not to do it
  \item If you are really not sure, you can ask someone (like me)
  \end{itemize}
\end{enumerate}
...
\section{Prescriptive and Descriptive Grammar}
...
\item PRESCRIPTIVE
  ...
  \item Identify speaker’s socioeconomic class \& education level
  \item Identify level of formality of a  particular usage
  \end{itemize}
\item DESCRIPTIVE
  \begin{itemize}
  \item Understand how
...
\end{exe}
\myslide{Descriptive Grammar: a Czech example}
\MyLogo{Knowledge you were never explicitly taught}
\begin{exe}
\ex \cs{Dej mi ho!}                 \hfill `Give it to me!'
\ex \cs{Petr mi ho dal.}            \hfill `Petr gave it to me.'
\ex \cs{Dej ho Petrovi!}            \hfill `Give it to Petr!'
\ex *\cs{Dej ho mi!}                \hfill intended: `Give it to me!'
\end{exe}
...
\myslide{Descriptive Grammar: an English example}
...
\begin{itemize}
\item \txx{Competence}:  mental knowledge that a speaker or listener has of language.
\item \txx{Performance}: the actual use of language in concrete situations
\end{itemize}
...
\begin{itemize}
\item By the mid-20th century, linguistics already had strong
structuralist traditions in Europe and North America,
including the Prague School.

\item From the late 1950s, Noam Chomsky’s work introduced
a new, explicitly formal approach to syntax and helped
make grammatical theory a major focus of linguistics.

\item Generative grammar became enormously influential,
although contemporary linguistics now includes many
competing theoretical traditions.

\item HPSG grew out of this broader formal and generative
tradition, but differs substantially from Chomsky’s
approach in its assumptions about grammar and
...
%\rule{0ex}{12ex}
  \begin{forest}
  cat, for tree={font=\scriptsize, s sep=2mm, l sep=4mm}
  [{Early Transformational Grammar (1955-1964)}
    [{Standard Theory (1964-1967)}
      [{EST (1967-1977)}
        [{\gr{REST}{1977-1981}}
          [{\gr{GB}{1981-1993}}
            [{\gr{MP}{1993-present}}]]]
        [{\gr{GPSG}{1979-1985}}
          [{\gr{HPSG}{1986-present}}, name=hpsg
            [{\gr{Sign-based CxG}{2001-present}}]]]
        [{\gr{Realistic TG}{1978-1980}}
          [{\gr{LFG}{1980-present}}, name=lfg]]]]
    [{Generative Semantics (1966-1975)}, name=gs
      [{Relational Grammar (1974-present)}
        [{Arc Pair Grammar (1980)}]]]
  ]
  \draw[dashed] (lfg) -- (hpsg);
  \draw[dashed] (lfg) to[out=0,in=180] (gs);
  \end{forest}

\myslide{Many Other Theories}
\begin{itemize}
\item Dependency Grammar  (links words not phrases)
\item Combinatory Categorial Grammar (allows multiple derivations)
...
\item Functional Grammar (considers function to be central)
  ...
  \end{itemize}
\item Semiotics  (how signs form  systems, and how individuals use signs to produce and communicate meaning)
\item Biosemiotics (how living organisms produce, interpret, and exchange
  signs and meanings)
\end{itemize}
...
\item I teach HPSG because I know it well
  \begin{itemize}
  \item I know HPSG because I sat next to Tsuneko Nakazawa at NTT
...
\begin{itemize}
\item Structure carries meaning --- the words alone do not
  \begin{exe}
  \ex \eng{Kim watched the student with the telescope} \com{who had it?}
  \ex \eng{Visiting relatives can be boring} \com{who visits whom?}
  \end{exe}
\item The generalizations are not obvious until you look
  \begin{exe}
  \ex \eng{Who did you say left?}
  \ex \bad{Who did you say that left?}
  \end{exe}
  No one taught you this, and no dictionary records it
\item Anything that has to work with a language needs a grammar
  \\ documentation, teaching, dictionaries, grammar checkers, translation
  --- and working out what language models do and do not know
\end{itemize}
...
\item \txx{Morphemes} 
  \begin{itemize}
    ...
    \item The realisations of morphemes are called \txx{morphs}.\\
          Example: English plural morpheme \txx{[number pl]} can be \eng{-s}, \eng{-es}, \eng{-en}, \eng{$\emptyset$}: \eng{boy-s}, \eng{box-es}, \eng{ox-en}, \eng{sheep}.
...
\myslide{What is morphology? (follow up)}
  ...
  \begin{enumerate}
    ...
    \item Derivational, inflectional and compositional processes of word formation occurring in a specific language.\\
          Example: \eng{German has a richer morphology than English.}
...
\myslide{Inflectional Morphology}
  \begin{itemize}
    ...
    \item It marks grammatical (\txx{morphosyntactic}) distinctions:
      \begin{description}
        \item[\txx{Conjugation} (verbal):] person, number, gender; tense, aspect, mood; agreement
        \item[\txx{Declension} (nominal):] case, number, gender, degree, definiteness
      \end{description}
...
\myslide{Inflection — paradigm}
  ...
  \begin{tabular}{lllll}
    ...
    1. & \eng{dehn-e}  & \eng{dehn-en} & \eng{dehn-te}    & \eng{dehn-te-n} \\
    2. & \eng{dehn-st} & \eng{dehn-t}  & \eng{dehn-te-st} & \eng{dehn-te-t} \\
    3. & \eng{dehn-t}  & \eng{dehn-en} & \eng{dehn-te}    & \eng{dehn-te-n} \\
  \end{tabular}
  \end{center}
%% end
  ...
  \begin{itemize}
    ...
    \item They may change:
      \begin{enumerate}
        ...
        \item \txx{valency of a verb}, e.g. Indonesian
          \idn[fall]{jatuh} $\rightarrow$ \idn[drop, cause to fall]{men+jatuh+kan}
          \com{\gm{caus} \idn{-kan} adds an argument}
        \item and more
      \end{enumerate}
  \end{itemize}
%% end
...
\item \txx{Acronym}: \eng{laser} ``Light Amplification by Stimulated Emission of Radiation''
\item \txx{Initialism}: \eng{HPSG} ``Head-Driven Phrase Structure Grammar'', \txx{UPOL}: ``U[niverzita] P[alackého] [v] Ol[omouci]'', \eng{\zh{東大}} ``\zh{東京大学}''
...
\begin{itemize}
\item We describe the grammar people have, not the grammar they are told
  to have
\item \txx{Competence} is what you know, \txx{performance} is what you do
  with it
\item Syntax has been central to linguistics since the 1950s
  \\ the theories since then form a family --- we work in one branch, HPSG
\item \txx{Morphology} supplies the pieces that syntax combines
\end{itemize}

\medskip

\begin{itemize}
\item Next week: Some Simple Theories of Grammar and Morphology (SWB Ch 2)
\item \emp{Problem Set 1} due Monday 23:59
\end{itemize}
