...
\begin{document}
\avmfont{\it}
\header{Wrap Up}{What have we learned}{}
\maketitle
...
\item We have seen how we can model syntax formally, using techniques of unification that are generally used in knowledge processing
\item Building explicit models is a useful skill for many tasks
\end{itemize}

\myslide{You also learned LaTeX}

LaTeX is a typesetting system that gives you fine grained control of your document.

\begin{itemize}\addtolength{\itemsep}{-1ex}
\item Allows separation of content and style
\item Good for mathematical notation, linguistic examples and more
\item Consistent handling of intra-document references and bibliographies
\item Works well with revision control systems (such as github)
\item Integrates well with program results
\item Can produce beautiful tables and illustrations
\item Good handling of multiple languages
\end{itemize}

It takes some effort to master, but makes some tasks much, much easier, especially long documents with many figures, \ldots
