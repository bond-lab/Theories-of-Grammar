...
\begin{document}
\header{Lecture 1b}{First attempts at a theory of grammar 
  and three approaches to morphology}{}\maketitle
%\include{schedule}
\MyLogo{CFG and Morphology}
...
\item Three approaches to morphology
  \begin{itemize}
  \item Item-and-Arrangement (IA) 
  \item Item-and-Process (IP)
  \item Word-and-Paradigm (WP)
  \end{itemize}
\end{itemize}

\section{Modelling Syntax \\ as Context Free Grammar}

\myslide{What makes a good model?}
\MyLogo{\textit{All grammars leak. \citep[p39]{sapir:1921} }}
...
\begin{center}
  ...
  And language is not perfectly regular \\
...
\begin{center} 
  \eng{I cured the dog with fleas}
\end{center}
\noindent\begin{tabular}{cc}
           PP under VP & PP under NP \\
\begin{minipage}{0.45\linewidth}
\begin{center} \small
\wtree{[S [NP [N [I]]]
          [VP [V [cured]]
              [NP [D [the]] [N [dog]]]
              [PP [P [with]] [NP [N [fleas]]]]]]}
\end{center}
\end{minipage}
&
\begin{minipage}{0.45\linewidth}
...
\myslide{Identifying constituents (nodes in the trees)}
\begin{block}{}
  \begin{itemize}
  \item
...
\myslide{Trees and Rules}
  \ctree{[$C_0$ [$C_1$] [\ldots] [$C_n$]]}is a well-formed nonlexical tree if (and only if)
\begin{itemize}
...
%%% ---------------------------------------------------------------
%%% Bottom-up: one slide per step, the tree grows as you go
%%% ---------------------------------------------------------------
...
\step{Start from the words}
\begin{center}
  \wtree{[D [the]]}\quad \wtree{[N [dog]]}\quad \wtree{[V [chased]]}\quad
  \wtree{[D [the]]}\quad \wtree{[N [cat]]}
\end{center}

\myslide{Bottom-up Tree Construction}
\step{NP \into D N}
\begin{center}
  \wtree{[NP [D [the]] [N [dog]]]}\quad \wtree{[V [chased]]}\quad
  \wtree{[D [the]]}\quad \wtree{[N [cat]]}
\end{center}
...
\step{NP \into D N}
\begin{center}
  \wtree{[NP [D [the]] [N [dog]]]}\quad \wtree{[V [chased]]}\quad \wtree{[NP [D [the]] [N [cat]]]}
\end{center}

\myslide{Bottom-up Tree Construction}
\step{VP \into V NP}
\begin{center}
  \wtree{[NP [D [the]] [N [dog]]]}\quad
  \wtree{[VP [V [chased]] [NP [D [the]] [N [cat]]]]}
\end{center}

\myslide{Bottom-up Tree Construction}
\step{S \into NP VP}
\begin{center}
  \wtree{[S [NP [D [the]] [N [dog]]]
          [VP [V [chased]] [NP [D [the]] [N [cat]]]]]}
\end{center}

%%% ---------------------------------------------------------------
%%% Top-down: the same tree, built from the start symbol instead
%%% ---------------------------------------------------------------
\myslide{Top-down Tree Construction}
\step{Start from the start symbol}
\begin{center} \ctree{[S]} \end{center}

\myslide{Top-down Tree Construction}
\step{S \into NP VP}
\begin{center} \ctree{[S [NP] [VP]]} \end{center}

\myslide{Top-down Tree Construction}
\step{NP \into D N}
\begin{center} \ctree{[S [NP [D] [N]] [VP]]} \end{center}

\myslide{Top-down Tree Construction}
\step{VP \into V NP}
\begin{center} \ctree{[S [NP [D] [N]] [VP [V] [NP]]]} \end{center}

\myslide{Top-down Tree Construction}
\step{NP \into D N}
\begin{center} \ctree{[S [NP [D] [N]] [VP [V] [NP [D] [N]]]]} \end{center}

\myslide{Top-down Tree Construction}
\step{Attach the words}
\begin{center}
  \wtree{[S [NP [D [the]] [N [dog]]]
          [VP [V [chased]] [NP [D [the]] [N [cat]]]]]}
\end{center}
\myslide{A CFG is declarative}

It says what is a possible structure not how that structure is made.

\begin{itemize}
\item \txx{Bottom-up}: string \into tree  (parsing)
\item \txx{Top-down}: tree \into string  (generation)
...
\item Techniques for processing CFGs are well understood.  It's
  computationally tractable (parsing is $O(n^3)$, or even linear for
  the deterministic subsets used in compilers). As well as natural
  language, we use them for programming languages, data formats (JSON,
  internet protocols), RNA secondary structure, machine translation,
  and constraining LLM output, \ldots
\end{itemize}

\myslide{Therefore \ldots}
...
\myslide{Of course, but it gets messy}

\begin{itemize}
\item S \into Vi 
\item S \into Vt NP
\item Vi: arrive, eat, \ldots
\item Vt: devour, eat, \ldots
\end{itemize}
...
\begin{itemize}
\item Morpheme-based (\txx{Item-and-Arrangement (IA)}): \\ list morphemes + specify their linear arrangement.
  --- \eng{spiral + -ize + -er} 
\item Lexeme-based (\txx{Item-and-Process (IP)}): \\ stems undergo processes/rules
  --- \texttt{make-verb()}, \texttt{instrument-noun()}. 
\item Word-based morphology (\txx{Word-and-Paradigm (WP)}): \\ words as \txx{word-forms} in \txx{paradigms};
  --- realizational rules fill paradigm slots. 
\item Each model fits different language properties, \ldots
  \begin{itemize}
  \item Isolating Languages (like Chinese) have almost no morphology
  \item Agglutinative Languages (like Turkish) have easily separable morphemes
  \item Fusion languages (like Czech) nflectional morphemes are fused together
  \end{itemize}
 \end{itemize}
...
\item IA Strength: Transparent Affixation
    \begin{itemize}
    \item Agglutinative-style sequences: \txx{one function \tot one form}.
    \item Hungarian: suffix allomorphs by stem vowels (back vs.\ front). \\
      	\begin{tabular}{llll}
	\eng{olvas-} & ``read''	& \eng{ül-} & ``sit''\\
	\eng{olvas-unk}	& ``we read''	& \eng{ül-ünk}	& ``we sit''\\
	\eng{olvas-tok}	& ``you (pl) read''	& \eng{ül-tök}	& ``you (pl) sit''\\
	\eng{olvas-nak}	& ``they read''	& \eng{ül-nek}	& ``they sit''\\
	\end{tabular}
...
\begin{itemize}
  ...
  \item \eng{spiral} → \eng{spiralize} (\texttt{make-verb(}\eng{spiral}\texttt{)}) 
    \\\eng{spiralize}  → \eng{spiralizer} (\texttt{instrument-noun(}\eng{spiralize}\texttt{)})
...
\end{enumerate}      \newpage
  \item
Aktionsart (lexical aspect): stative, telic, punctual
  \item
Aspect:
\begin{enumerate}
...
f. & vyprodá & ``(s)he will sell off''\\
g. & vydám & ``I will hand out, give out''\\
h. & povyprodáme & ``we will sell out gradually''\\
i. & dáváš & ``you keep giving''\\
j. & dopovyprodáváš\,mi & ``you will gradually finish selling (it) out for me''\\
k. & dopovyprodáváme\,ti & ``we will gradually finish selling (it) out for you''\\
\end{tabular}
...
\textsc{asp} 	& \textsc{asp} 	& \textsc{asp} 	& \textsc{direction} 	& root 	& \textsc{iterative} 	& \textsc{subj}	& \textsc{number} 	& \textsc{obj}\\
 do- 	& po- & vy- & pro- ``for'' & dá 	& va-$\sim$-vá & -m (1sg) & -e (\textsc{pl}) & =mi (\textsc{1sg.dat}) \\
...
\myslide{Chapter 2, Problem 1 --- grammar}
\begin{tabular}{cc}
...
\myslide{Chapter 2, Problem 1 --- Questions}
...
\item Swiss German example:
  \begin{exe}
    ...
    \ldots we {the children-\gm{acc}} Hans-\gm{dat} the house-\gm{acc} let help paint \\
    \trans ``we let the children help Hans paint the house'' [gsw]
  \end{exe}
...
strings.
\item Shieber’s argument is that Swiss German is not \emp{weakly}
context-free and therefore not \emp{strongly} context-free.
\item Bresnan et al (1982) had already argued that Dutch
is not \emp{strongly} context-free, but the argument was
