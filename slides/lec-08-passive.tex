...
&
\begin{tiny}
\avm{< \textnormal{loved}, [\type*{part-lxm}\\
  SYN & [HEAD & [\type*{verb}\\
  AGR& {\1}\\                                
  FORM & pass]\\
  VAL & [SPR & < [AGR& {\1}]\ >]]\\
  ARG-ST & < NP$_j$ ( ,\ [\hfil PP\\[-1ex]
  [FORM & by\\
  INDEX & $i$]]\ ) >\\
  SEM & [INDEX & $s$\\
  RESTR & < [RELN & {\bf love}\\
...
\begin{tiny}
   \avm{< \textnormal{loved}, 
     ...
     VAL & [SPR & < {\2}[AGR\ {\1}{\,}] >\\
     COMPS & {\tag{B}}]]\\
     ARG-ST & < {\2}NP$_j$ >\ $\oplus$\ {\tag{B}} < ( ,\ [\hfil PP\\[-1ex]
     [FORM & by\\
     INDEX & $i$]]\ ) >\\
     SEM & [INDEX & $s$\\
     RESTR & < [RELN & {\bf love}\\
...
cat
[{S} [{\avmbox{1}NP}]
[{VP\makebox[0mm][l]{$[$ SPR $\langle \avmbox{1} \rangle ]$}} [{V\tiny \makebox[0mm][l]{$[$ SPR $\langle \avmbox{1} \rangle ]$}}]
[{VP\makebox[0mm][l]{$[$ SPR $\langle \avmbox{1} \rangle ]$}} [{V}]
[{PP} [{P}]
[{NP}]]]]]
\end{forest}
